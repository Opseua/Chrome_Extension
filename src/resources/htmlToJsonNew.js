...
async function htmlToJsonNew(inf) {
    ...
    try {
        if (!`rodar no →  NODEJS`.includes(engName)) { // [ENCAMINHAR PARA DEVICE]
            let infDevAndFun = { 'e': e, 'enc': true, 'data': { 'name': 'htmlToJsonNew', 'par': inf, 'retInf': inf.retInf } };
...
if (eng) { // CHROME
    window['htmlToJsonNew'] = htmlToJsonNew;
} else { // NODEJS
    global['htmlToJsonNew'] = htmlToJsonNew;
}
