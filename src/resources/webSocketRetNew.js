...
// const retWebSocketRet = webSocketRetNew(infWebSocketRet)
...
const variable = [
    'Password@2023WebSocketRet',
    {
        "retUrl": "ws://xx.xxx.xxx.xx:xx/######_RET",
        "retInf": "ID DO RETORNO",
        "name": "notification",
        "par": {
            "title": "titulo",
            "message": "texto"
...
async function webSocketRetNew(inf) {
    let ret = { 'ret': false };
    try {
        let WebS;
        ...
        if (!typeof variable == 'object' || variable.length == 1 || variable[0] !== 'Password@2023WebSocketRet') { console.log('PAROU') } else {
            variable.slice(1).map(async (value, index) => {
                console.log(value)
            })
        }
        // return
...
// if (typeof window !== 'undefined') { // CHROME
//     window['webSocketRetNew'] = webSocketRetNew;
// } else { // NODEJS
//     global['webSocketRetNew'] = webSocketRetNew;
// }


await webSocketRetNew()