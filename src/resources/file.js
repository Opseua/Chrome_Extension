    ...
    try {
        ...
        if (!inf.action || !['write', 'read', 'del', 'inf', 'relative', 'list', 'change', 'md5', 'isFolder', 'storage',].includes(inf.action)) { ret['msg'] = `FILE: ERRO | INFORMAR O 'action'`; }
            ...
            async function fileRelative(inf) {
                let resNew = { 'ret': false }; relative = inf.path; function runPath(pp, par) {
                ...
                }; let res = [`${eng && inf.functionLocal ? '' : `${letter}:/`}${runPath(inf.path, inf.functionLocal ? true : false)}`]; resNew['ret'] = true; resNew['msg'] = `FILE [RELATIVE]: OK`; resNew['res'] = res; return resNew
            }
            ...
            async function fileWrite(inf) {
                let resNew = { 'ret': false }, path; if (typeof inf.rewrite !== 'boolean') { resNew['msg'] = `FILE [WRITE]: ERRO | INFORMAR O 'rewrite' TRUE ou FALSE`; }
                    ...
                    if (inf.raw) { let infRawText = { 'e': e, 'obj': inf.text }; let retRawText = await rawText(infRawText); text = retRawText } else { // STRING /OBJETO / BUFFER
                        ...
                        if (path.includes('%/')) { path = path.split('%/')[1] } else if (path.includes(':')) { path = path.split(':/')[1] }; if (inf.rewrite) {
                        ...
                        if (path.split('/').length > 2) { await _fs.promises.mkdir(_path.dirname(path), { recursive: true }); }; await _fs.promises.writeFile(path, text, { flag: !inf.rewrite ? 'w' : 'a' })
            ...
            async function fileRead(inf) {
                ...
                else { infFile = { 'path': inf.path, 'functionLocal': inf.functionLocal }; retFile = await fileRelative(infFile); path = retFile.res[0] }; if (eng) { // CHROME
            ...
            async function fileDel(inf) {
                let resNew = { 'ret': false }, path; if (inf.path.includes(':')) { path = inf.path }
                else { infFile = { 'path': inf.path, 'functionLocal': inf.functionLocal }; retFile = await fileRelative(infFile); path = retFile.res[0] }; async function delP(inf) {
                    try {
                        let ok = false; try {
                    ...
                    } catch (catchErr) { let retRegexE = await regexE({ 'inf': inf, 'e': catchErr, }); resNew['msg'] = retRegexE.res; }
            ...
            async function fileIsFolder(inf) {
                let resNew = { 'ret': false }, path; try {
                    if (inf.path.includes(':')) { path = inf.path } else { infFile = { 'path': inf.path, 'functionLocal': inf.functionLocal }; retFile = await fileRelative(infFile); path = retFile.res[0] };
                    let res = _fs.statSync(path).isDirectory(); resNew['ret'] = true; if (!inf.listRead) { resNew['msg'] = `FILE [IS FOLDER]: OK`; resNew['res'] = res } else {
                        // USADO SOMENTE NO 'ARQUIVOS WEB' DO SEVIDOR | É PASTA [LISTAR] |  É ARQUIVO [LER]
                        if (res) { infFile = { 'e': e, 'action': 'list', 'functionLocal': false, 'path': path, 'max': inf.max }; retFile = await fileList(infFile); resNew = retFile }
                        else { resNew['msg'] = `FILE [IS FOLDER]: OK`; infFile = { 'path': path, 'functionLocal': inf.functionLocal && !eng ? true : false }; retFile = await fileRead(infFile); resNew = retFile }
                    }
            ...
            else if (inf.action == 'md5' && !eng) { return await fileMd5(inf) } else if (inf.action == 'isFolder' && !eng) { return await fileIsFolder(inf) } else if (inf.action == 'storage' && !eng) { return await fileStorage(inf) }
        }
