        ...
        if (!action || !['write', 'read', 'del', 'inf', 'relative', 'list', 'change', 'md5', 'isFolder', 'storage',].includes(action)) { ret['msg'] = `FILE: ERRO | INFORMAR O 'action'`; }
            ...
            async function fileWrite(inf = {}) {
                let { functionLocal, path, rewrite, text, raw, } = inf; let resNew = { 'ret': false }; if (typeof rewrite !== 'boolean') { resNew['msg'] = `FILE [WRITE]: ERRO | INFORMAR O 'rewrite' TRUE ou FALSE`; }
                    ...
                    if (raw) { let infRawText = { e, 'obj': text }; let retRawText = rawText(infRawText); text = retRawText } else { // STRING / OBJETO / BUFFER
                        ...
                        infFile = { 'path': path, 'functionLocal': functionLocal && !eng ? true : false }; retFile = await fileRelative(infFile); path = retFile.res[0]
                    }; if (eng) { // CHROME
                        if (path.includes('%/')) { path = path.split('%/')[1] } else if (path.includes(':')) { path = path.split(':/')[1] }; if (rewrite) {
                            try { infFile = { 'path': path, 'functionLocal': functionLocal && !eng ? true : false }; retFile = await fileRead(infFile); text = `${retFile.res || ''}${text} ` }
            ...
            async function fileDel(inf = {}) {
                ...
                if (!path.includes(':')) { infFile = { 'path': path, 'functionLocal': functionLocal }; retFile = await fileRelative(infFile); path = retFile.res[0] }; async function delP(txt) {
                    try {
                        let ok = false; try {
                            let s = await _fs.promises.stat(txt); if (s.isDirectory()) { let as = await _fs.promises.readdir(txt); for (let a of as) { let c = _path.join(txt, a); await delP(c) }; await _fs.promises.rmdir(txt) }
            ...
            async function fileIsFolder(inf = {}) {
                let { functionLocal, path, listRead, max = 200, } = inf; let resNew = { 'ret': false }; try {
                    ...
                    let res = _fs.statSync(path).isDirectory(); resNew['ret'] = true; if (!listRead) { resNew['msg'] = `FILE [IS FOLDER]: OK`; resNew['res'] = res } else {
                        ...
                        else { resNew['msg'] = `FILE [IS FOLDER]: OK`; infFile = { 'path': path, 'functionLocal': functionLocal && !eng ? true : false }; retFile = await fileRead(infFile); resNew = retFile }
                    }
            ...
            else if (action == 'relative') { return await fileRelative(inf) } else if (action == 'list' && !eng) { return await fileList(inf) } else if (action == 'change' && !eng) { return await fileChange(inf) }
            else if (action == 'md5' && !eng) { return await fileMd5(inf) } else if (action == 'isFolder' && !eng) { return await fileIsFolder(inf) } else if (action == 'storage' && !eng) { return await fileStorage(inf) }
        }
