...
async function configStorage(inf) {
    ...
    try {
        if (!eng && Array.isArray(inf) && inf.length == 1) { // ### CS
            ...
            let run = false;
            if (!inf.action || !['set', 'get', 'del'].includes(inf.action)) { ret['msg'] = `CONFIG STORAGE: ERRO | INFORMAR O 'action'`; }
            else {
                if ((!inf.key || inf.key == '')) { ret['msg'] = `CONFIG STORAGE: ERRO | INFORMAR A 'key'`; }
                else { if (inf.action == 'set' && !inf.value) { ret['msg'] = `CONFIG STORAGE: ERRO | INFORMAR O 'value'`; } else { run = true } }
            }; if (run) {
                if (eng) { // CHROME
                        ...
                        await storageSet(inf); async function storageSet(inf) {
                            return new Promise((resolve) => {
                                let data = {}; data[inf.key] = inf.value; chrome.storage.local.set(data, async () => {
                                    ...
                                    else { ret['msg'] = 'STORAGE SET: OK'; ret['ret'] = true; }; resolve(ret);
                                });
                            });
                        }
                                            ...
                                            if (config[inf.key]) {
                                                let data = {}; data[inf.key] = config[inf.key]; return new Promise((resolve) => {
                                                    chrome.storage.local.set(data, async () => {
                                                        if (chrome.runtime.lastError) { ret['msg'] = `STORAGE [SET*]: ERRO | ${chrome.runtime.lastError}`; }
                                                        else { ret['res'] = config[inf.key]; ret['msg'] = 'STORAGE GET: OK'; ret['ret'] = true; }; resolve(ret)
                                                    });
                                                })
                                            } else { ret['msg'] = `STORAGE [GET]: ERRO |  CHAVE '${inf.key}' NAO ENCONTRADA`; }
                                        }; await checkConfig()
                                    } else { ret['res'] = result[inf.key]; ret['msg'] = 'STORAGE GET: OK'; ret['ret'] = true; }; resolve(ret);
                                });
                            });
                        }
                        ...
                        await storageDel(inf); async function storageDel(inf) {
                            return new Promise((resolve) => {
                                chrome.storage.local.get(inf.key, async (result) => {
                                    ...
                                    else if (Object.keys(result).length == 0) { ret['msg'] = `STORAGE [DEL]: ERRO | CHAVE '${inf.key}' NAO ENCONTRADA`; }
                                    else { chrome.storage.local.remove(inf.key, async () => { }); ret['msg'] = 'STORAGE DEL: OK'; ret['ret'] = true; }; resolve(ret)
                                }); return
                            });
                        }
                    }
                    ...
                    if (ret_Fs) { let configFile = await _fs.promises.readFile(path, 'utf8'); config = JSON.parse(configFile) } else { config = {} }; if (!inf.key || inf.key == '') { ret['msg'] = `CONFIG: ERRO | INFORMAR A 'key'`; }
                    else if (inf.action == 'set') { // #### CONFIG: SET
                        if (!inf.value && !inf.value == false) { ret['msg'] = `CONFIG: ERRO | INFORMAR O 'value'`; } else {
                            if (inf.key == '*' && typeof inf.value !== 'object') { ret['msg'] = `CONFIG: ERRO | VALOR NAO É OBJETO`; } else if (inf.key == '*') { config = inf.value } else { config[inf.key] = inf.value };
                            if (!ret.msg) {
                                infFile = { 'e': e, 'action': 'write', 'path': path, 'rewrite': false, 'text': JSON.stringify(config, null, 2) }; retFile = await file(infFile); ret['msg'] = `CONFIG SET: OK`; ret['ret'] = true;
                            }
                        }
                    } else if (inf.action == 'get') { // #### CONFIG NODE: GET
                        if (!ret_Fs) { ret['msg'] = `CONFIG [GET]: ERRO | ARQUIVO '${path}' NAO ENCONTRADO`; } else if (inf.key == '*' || (inf.key !== '*' && config[inf.key])) {
                            ret['res'] = inf.key == '*' ? config : config[inf.key]; ret['msg'] = `CONFIG GET: OK`; ret['ret'] = true;
                        } else { ret['msg'] = `CONFIG [GET]: ERRO | CHAVE '${inf.key}' NAO ENCONTRADA`; }
                    } else if (inf.action == 'del') { // #### CONFIG NODE: DEL
                        if (!ret_Fs) { ret['msg'] = `CONFIG [DEL]: ERRO | ARQUIVO '${path}' NAO ENCONTRADO`; } else if (config[inf.key]) {
                        ...
                        } else { ret['msg'] = `CONFIG [DEL]: ERRO | CHAVE '${inf.key}' NAO ENCONTRADA`; }
                    }
                }
            }
        }
