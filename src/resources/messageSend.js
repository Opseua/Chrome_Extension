...
let filaBigFalse = []; let filaBigTrue = []; let sending = false; function enviarMensagem(inf) {
    ...
    if (filaBigFalse.length > 0) { return { big: false, value: filaBigFalse.shift() }; } else if (filaBigTrue.length > 0) { return { big: true, value: filaBigTrue.shift() }; }
    ...
    while (true) {
        ...
        let { messageId, buffer, partesRestantes, secondsAwait } = value; let message = JSON.stringify(value);
...
async function messageSend(inf) {
    ...
    if (typeof inf.message === 'object') {
        message = JSON.stringify(inf.message); buffer = message.includes(`"type":"Buffer"`) && message.includes(`"data":[`) ? true : false
    ...
    messageId = secondsAwait == 0 ? `${messageId}` : `${messageId}_RET-TRUE`; let { resWs, wsClients } = inf; let host = resWs.host, room = resWs.room, isSerCli = wsClients ? 'isSer' : 'isCli'
    ...
    // LISTENER DE STATUS: MONITORAR (SE NECESSÁRIO)
    if (secondsAwait > 0) {
        retAwaitTimeout = await retAwaitTimeout;
