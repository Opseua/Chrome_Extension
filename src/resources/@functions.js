...
// ## resources
// await import('./notification.js');  await import('./promptChrome.js'); await import('./getCookies.js'); await import('./chromeActions.js');
// await import('./getPage.js'); await import('./sniffer.js'); await import('./tabSearch.js'); await import('./commandLine.js');
await import('./excel.js'); // await import('./splitText.js');
// ## scripts
    ...
    try { // PASSAR NO jsonInterpret
        ...
        if (!inf.action || !['write', 'read', 'del', 'inf', 'relative', 'list', 'change'].includes(inf.action)) {
            ...
            if (inf.action == 'write') { // ########################## WRITE
                ...
                e = JSON.stringify(new Error().stack).replace('at ', ''); if (conf.length == 1) { // NOME DO PROJETO E TERMINAL
                    if (typeof window !== 'undefined') { // CHROME
...
async function sniffer(inf) {
    ...
    if (typeof window == 'undefined') { // [ENCONTRAR DEVICE] NODEJS
        const infDevAndFun = { 'wsUrl': wsUrl, 'name': 'sniffer', 'retUrl': inf.retUrl, 'par': { 'newReqSend': inf.newReqSend, 'arrUrl': inf.arrUrl } }
        const retDevAndFun = await devAndFun(infDevAndFun); return retDevAndFun
    }; return new Promise(resolve => {
    ...
    try {
        if (typeof window == 'undefined') { // [ENCONTRAR DEVICE] NODEJS
            const infDevAndFun = {
                ...
                'par': { 'search': inf.search, 'openIfNotExist': inf.openIfNotExist, 'active': inf.active, 'pinned': inf.pinned, 'url': inf.url }
            }; const retDevAndFun = await devAndFun(infDevAndFun); return retDevAndFun
