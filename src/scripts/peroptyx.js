...
async function peroptyx(inf) {
    let ret = { 'ret': false };
    try {
        let infSniffer, retSniffer, infNotification, retNotification, infChatGpt, retChatGpt, infClipboard, retClipboard, infTranslate1, retTranslate1, infTranslate2, retTranslate2
        const gOEve = async (i) => {
            if (i.inf.sniffer === 2) { gORem(gOEve); chrome.browserAction.setBadgeText({ text: '' }); ret = { 'ret': false }; return ret }
        }; gOAdd(gOEve);
        async function run() {
            ret = { 'ret': false }
            infSniffer = { 'newReqSend': true, 'arrUrl': ['https://www.tryrating.com/api/survey'] }
            ...
            if (!retSniffer.tasks[0].taskData.hasOwnProperty('testQuestionInformation')) {
                ...
                const res = await Promise.all(resultList.map(async (v, index) => {
                    ...
                    if (comentario) {
                        ...
                        infChatGpt = { 'provider': 'ora.ai', 'input': `REWRITE THIS SENTENCE WITH OTHER WORDS, KEEPING THE SAME MEANING:\n\n ${comentario}` }
